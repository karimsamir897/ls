...
const prompt = `Runs JavaScript code to interact with the Ls API API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    LS_API_BEARER_TOKEN: requireValue(
      readEnv('LS_API_BEARER_TOKEN') ?? client.bearerToken,
      'set LS_API_BEARER_TOKEN environment variable or provide bearerToken client option',
    ),
    LS_API_BASE_URL: readEnv('LS_API_BASE_URL') ?? client.baseURL ?? undefined,
  };
