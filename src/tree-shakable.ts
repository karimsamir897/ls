...
import { BaseLsAPI, ClientOptions } from './client';
import { APIResource } from './core/resource';
import { hasOwn } from './internal/utils/values';

export type PartialLsAPI<T extends MaybeResource> = BaseLsAPI & T;
 ...
 * bundle size.
 * Import the resource classes you need from `ls-api-mcp/resources/*`.
 * Use the BaseResource variants if you do not need to use subresources.
 *
 * @example
 * ```ts
 * import { V1 } from `ls-api-mcp/resources/v1`;
 * import { createClient } from `ls-api-mcp/tree-shakable`;
 *
...
): PartialLsAPI<InferClientStructure<T>> {
  const client = new BaseLsAPI(options);
